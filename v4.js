  ...
  const parsedJson = JSON.parse(jsonOutput);
    ...
    img.onload = () => {
      ...
      parsedJson.forEach(item => {
          if (item.type === 'image') { //Only cut image assets
...
cutAssetsButton.addEventListener('click', async () => {
    ...
    if (!apiKey) {
     ...
     const refinedDescription = refinedDescriptionContainer.textContent;
       showLoading();
    try{
           const assets =  await cutAssets(uploadedImageFile, refinedDescription);
